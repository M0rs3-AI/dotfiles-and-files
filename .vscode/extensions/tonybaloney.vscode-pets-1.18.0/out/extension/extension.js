...
async function handleRemovePetMessage(message) {
    ...
    await vscode.window
        ...
        placeHolder: localize.localize('selectPetToRemove', 'Select the pet to remove.'),
    })
        .then((pet) => {
        if (pet) {
            const panel = getPetPanel();
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vscode-pets.spawn-pet', async () => {
        ...
        if (panel) {
            const selectedPetType = await vscode.window.showQuickPick(localize.stringListAsQuickPickItemList(types_1.ALL_PETS), {
                placeHolder: localize.localize('selectPet', 'Select a pet'),
            });
            ...
            switch (selectedPetType.value) {
                ...
                case "cat" /* PetType.cat */:
                    ...
                    var selectedColor = await vscode.window.showQuickPick(localize.stringListAsQuickPickItemList(choices), {
                        placeHolder: localize.localize('selectColor', 'Select a color'),
                    });
            ...
            const name = await vscode.window.showInputBox({
                placeHolder: localize.localize('leaveBlank', 'Leave blank for a random name'),
            ...
            if (!spec.type || !spec.color || !spec.size) {
                return vscode.window.showWarningMessage(localize.localize('cancelledSpawningPet', 'Cancelled Spawning Pet'));
            }
            else if (spec) {
                panel.spawnPet(spec);
            }
            ...
            createPetPlayground(context);
            vscode.window.showInformationMessage(localize.localize('spawnPetWithClosedPlayground', "A Pet Playground has been created. You can now use the 'Spawn Additional Pet' Command to add more pets."));
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand('vscode-pets.remove-all-pets', () => {
        ...
        if (panel !== undefined) {
            ...
            createPetPlayground(context);
            vscode.window.showInformationMessage(localize.localize('removePetsWithClosedPlayground', "A Pet Playground has been created. You can now use the 'Remove All Pets' Command to remove all pets."));
        }
    }));
...
class PetPanel extends PetWebviewContainer {
    ...
    static createOrShow(extensionUri, extensionPath, petColor, petType, petSize, theme, themeKind, throwBallWithMouse) {
        ...
        const panel = vscode.window.createWebviewPanel(PetPanel.viewType, localize.localize('petPanel', 'Pet Panel'), vscode.ViewColumn.Two, getWebviewOptions(extensionUri));
