	...
	var webViewProvider = {
		resolveWebviewView: async function (thisWebview) {
			try {
				...
				thisWebview.webview.onDidReceiveMessage(
					async (data) => {
						...
						if (data.command) {
							...
							else if (data.command=='features') open('https://www.useblackbox.io/pricing')
						}
					},
					undefined,
					_.subscriptions
				...
				function recieveMessage(roomId, by, type = "message", message) {
					addToRoomMessages(roomId, by, type, message)
				...
				<div class="chat-connected">
					...
					<div class="lobby active">
						...
						<div class="all-chats">
							...
							<div style="text-align: center"><span style="font-style:italic;" center="">Blackbox COMMS is built to connect developers. Post your questions in one of these groups and other developers will be reading your questions and responding.</span></div>
				...
				(function() {
					...
					function sendMessage(
						...
						if (str) {
							...
							holder.classList += "chatMessage sent"
					...
					function recieveMessage(str, from, time = null) {
						const holder = document.createElement("div")
						holder.classList += "chatMessage recieved"
					...
					window.addEventListener('message', event => {
						...
						if(data.command === "recieveVscodeMessage"){
							recieveMessage(data.message, data.from, data.time)
						}
							...
							chats.forEach((chat)=>{
								if(chat.getAttribute("data-id") === roomId){
									if ( chat.querySelector(".badge") ) chat.querySelector(".badge").textContent = data.badge
								}
							})
							...
							if(messages){
								messages.forEach((el)=>{
									if(el.type === "message"){
										if(el.by === username){
											sendMessage(el.message, el.time, false)
										}
										else{
											recieveMessage(el.message, el.by, el.time, false)
										}
			...
			<body>
				...
				<div style='display: none'>
					...
					<div class="btn-default btn-features">
						<span class="btn-autocomplete">Try for Free</span>
					</div>
				</div>
				<script>
					(function() {
